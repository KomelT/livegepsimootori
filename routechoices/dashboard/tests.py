...
class EssentialDashboardBase(APITestCase):
    def setUp(self):
        self.client = APIClient(HTTP_HOST="dashboard.track.meshtrail.si")
    ...
    def reverse_and_check(
        ...
        self.assertEqual(url, f"//dashboard.track.meshtrail.si{expected}")
        return url
    ...
    def test_edit_map(self):
        ...
        res = self.client.post(
            url,
            {
                "name": "My Test Map",
                "image": map_image,
                "calibration_string_raw": "61.45075,24.18994,61.44656,24.24721,61.42094,24.23851,61.42533,24.18157",
            },
        )
        self.assertEqual(res.status_code, status.HTTP_302_FOUND)
        ...
        res = self.client.post(
            url,
            {
                "name": "My Test Map",
                "image": map_image,
                "calibration_string_raw": "61.45075,24.18994,61.44656,24.24721,61.42094,24.23851,61.42533,24.18157,123",
            },
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertContains(res, "invalid-feedback")
        ...
        res = self.client.post(
            url,
            {
                "name": "My Test Map",
                "image": map_image,
                "calibration_string_raw": "61.45075,24.18994,61.44656,24.24721,61.42094,24.23851,61.42533,xx.18157",
            },
        )
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertContains(res, "invalid-feedback")
...
class TestInviteFlow(APITestCase):
    ...
    def test_send_invite_existing_user(self):
        ...
        res = self.client.post(accept_link)
        self.assertEqual(res.status_code, status.HTTP_302_FOUND)
        self.assertRedirects(res, "https://dashboard.track.meshtrail.si/login")
        ...
        self.assertRedirects(
            res,
            "https://dashboard.track.meshtrail.si/login",
            target_status_code=status.HTTP_302_FOUND,
        )
